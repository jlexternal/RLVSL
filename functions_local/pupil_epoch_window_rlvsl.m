function [epoch_window,excluded_trials,fbs,rsp,qts,trs,bks,cds] = pupil_epoch_window_rlvsl(subjlist,cfg)
...
for isubj = subjlist
    ...
    if subj_init
        ...
        nb = length(expe);
        excluded_trials  = false(nt,nb-3,numel(subjlist)); % logical matrix of excluded trials
        fbs = nan(nt,nb-3,numel(subjlist)); % matrix of feedback values
        rsp = nan(size(fbs));   % subject responses
        qts = nan(size(fbs));   % matrix of quarters
        cds = nan(size(fbs));   % matrix of conditions
    ...
    for ib = 1:nb
        ...
        for it = 1:nt
            ...
            cds(it,ctr_blck,ctr_subj) = condtype;
...
trs = repmat((1:nt)',[1 nb-3 numel(subjlist)]);
bks = repmat((1:nb-3),[nt 1 numel(subjlist)]);
