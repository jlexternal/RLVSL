...
for ifit = 1:nfits
    ...
    for ib = 1:out_sim{ifit}.sim_struct.expe(1).cfg.nbout
        choices(ib,:) = out_sim{ifit}.sim_struct.expe(ib).resp;
        outcome(ib,:) = out_sim{ifit}.sim_struct.expe(ib).blck_trn+50;
    end
...
recpars = nan(nfits,3);
for ifit = 1:nfits
    genpars(ifit,1) = out_sim{ifit}.sim_struct.sim.cfg_model.zeta;
    genpars(ifit,2) = out_sim{ifit}.sim_struct.sim.cfg_model.alpha;
    genpars(ifit,3) = out_sim{ifit}.sim_struct.sim.cfg_model.strpr;
    
    recpars(ifit,1) = out_fit{ifit}.zeta;
    recpars(ifit,2) = out_fit{ifit}.alpha;
    recpars(ifit,3) = out_fit{ifit}.priorpar;
end

for ipar = 1:3
    figure(ipar);
    scatter(genpars(:,ipar),recpars(:,ipar));
end
