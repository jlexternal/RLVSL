function sim_out = sim_noisyKF_fn(cfg)
% sim_noisyKF_fn
%
% Simulates the noisy Kalman filter model for experiment RLVSL
...
% Required configuration values
    ...
    sbias_ini   = cfg.sbias_ini;    % KF means biased toward the subject
    ...
    % Model parameters (ns x 1 vectors)
...
for ib = 1:nb
    for it = 1:nt
        if it == 1
            ...
            if sbias_ini
                % initial tracking mean biased toward subject choice
                mt(ib,it,rb,:)   = ms;
                mt(ib,it,3-rb,:) = 1-ms;
            else
            ...
            vt(ib,it,:,:) = repmat(reshape(kini./(1-kini)*vs,[1 1 1 ns]),[1 1 2 1]);
        ...
        for io = 1:2
            ...
            switch nscheme
                case 'rpe' % RPE-scaled learning noise
                    st(ib,it,c,ind_c) = sqrt(reshape(zeta(ind_c),size(rew_seen)).^2.*(rew_seen-mt(ib,it-1,c,ind_c)).^2+ksi^2); 
                case 'upd' % update-value-scaled learning noise
                    st(ib,it,c,ind_c) = sqrt(reshape(zeta(ind_c),size(rew_seen)).^2.*(reshape(kt(c,ind_c),size(rew_seen)).*(rew_seen-mt(ib,it-1,c,ind_c)).^2+ksi^2)); 
            end
            ...
            switch lscheme
                ...
                case 'sym'
                    ...
                    switch nscheme
                        case 'rpe'
                            st(ib,it,u,ind_c) = sqrt(reshape(zeta(ind_c),size(rew_unseen)).^2.*((rew_unseen-mt(ib,it-1,u,ind_c)).^2+ksi^2));
                        case 'upd'
                            st(ib,it,u,ind_c) = sqrt(reshape(zeta(ind_c),size(rew_unseen)).^2.*reshape(kt(u,ind_c),size(rew_unseen)).*((rew_unseen-mt(ib,it-1,u,ind_c)).^2+ksi^2));
                    end
        ...
        vt(ib,it,:,:)  = vt(ib,it,:,:)+...
                            arrayfun(@(k)fzero(@(v)fk(v)-min(max(k,0.001),0.999),vs.*2.^[-30,+30]),...
                                     repmat(reshape(kinf,[1 1 1 ns]),[1 1 2 1]));

        % decision variable stats
        switch cscheme
            case 'qvs'
                md = reshape(mt(ib,it,1,:)-mt(ib,it,2,:),[1 ns]);
                sd = reshape(sqrt(sum(st(ib,it,:,:).^2,3)+reshape(ssel.^2,[1 1 1 ns])),[1 ns]);
            case 'ths'
                md = reshape((mt(ib,it,1,:)-mt(ib,it,2,:))./sqrt(sum(vt(ib,it,:,:))),[1 ns]);
                sd = reshape(sqrt(sum(st(ib,it,:,:).^2,3)./sum(vt(ib,it,:,:),3)+reshape(ssel.^2,[1 1 1 ns])),[1 ns]);
        end
