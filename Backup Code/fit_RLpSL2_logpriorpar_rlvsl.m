...
%    * prior  - the structure learned prior bias at each decision (logit)
%
...
% 3/ structure learned prior (in units of logit)
pnam{1,3} = 'prior';
pmin(1,3) = -4;
pmax(1,3) = 4;
pfun{1,3} = @(x)normpdf(x,0,3);
pini(1,3) = normstat(0,3);
pplb(1,3) = norminv(0.15,0,3);
ppub(1,3) = norminv(0.85,0,3);
...
function [ll,p,e,q,z,k,v] = getll(zeta,alpha,prior,tau,ksi)
    ...
    for itrl = 1:ntrl
        ...
        slt(itrl,:) = prior;                                     % logit contribution of SL 
