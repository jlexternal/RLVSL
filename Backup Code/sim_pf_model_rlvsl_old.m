function objFn = sim_pf_model_rlvsl_old(params)
...
%           : log(p(params|data))
%
% Version:  This version of sim_of_model_rlvsl samples decisions based on the
%           weights determined by the action softmax (with inverse temperature beta).
%           The ancestral weights are determined by this same softmax, even when the
%           action does not correspond to data. 
%           - Jan 17 2020
tic;

% Output structure containing several variables to be examined when errors are thrown
global errorVar
errorVar = struct;
...
for ib = 1:nb
  ...
  % Prior belief and SL (Beta distribution) updating on first trial for each block
    % 1st and 2nd blocks (in a given condition) have flat priors
    if ib_c(ic) < 3    
        % Initialization of particles and their values
    ...
    elseif ib_c(ic) >= 3 % consider switch/stay-based prior from Beta distribution
        ...
        mp(ib_c(ic),1,:,ic) 	= (1+exp(-(q(ib_c(ic),1,:,ic)+s(ib_c(ic),1,:,ic)))).^-1;                             % Estimate probability of the shape
    end
    
    % subsequent trials
    for it = 1:nt
        fprintf('ic = %d, ib_c = %d, it = %d \n',ic,ib_c(ic),it);
        if it > 1
            % Structure learning propagation from ancestors
            ...
            % Incorporate both RL and SL toward belief
            mp(ib_c(ic),it,:,ic) 	= (1+exp(-(q(ib_c(ic),it,:,ic)+s(ib_c(ic),1,:,ic)))).^-1;
        end
        ...
        %   dv  - data provided by simulation
        %   wdv - likelihood of the decision variable (softmax on the signal leading to action)
        %   lse - log-sum-exp on wdv
        %   wts - weights given to particles for propagation based on their likelihoods
        ...
        % Probability of action taken by particles corresponding to data
        ...
        % Assign propagation weights to particles via log-softmax
        wts(ib_c(ic),it,:,ic)   = log(wdv(ib_c(ic),it,:,ic)/sum(wdv(ib_c(ic),it,:,ic)));
        wts_sum = sum(wts(ib_c(ic),it,:,ic));
        wts(ib_c(ic),it,:,ic)   = wts(ib_c(ic),it,:,ic)./wts_sum;

        % debug code
        if      ismember(1,isnan(lse(ib_c(ic),it,ic)))
            % lse<wdv<mp<q;s<prl;psl<mt,vt;abs
            errorout;
            error('found NaN in objective function calculation @ ic = %d, ib_c = %d, it = %d \n',ic,ib_c(ic),it);
        ...
        if sum(rs_mult) < np
            ...
            % Structure learning propagation from last-trial ancestor
            if it == 1
                mt_anc_beta(ib_c(ic),it,rs_part_ct+1:n_impov,ic) = mt_anc_beta(ib_c(ic)-1,end,imps,ic);
            else
        ...
        for ia = 1:length(ind_anc)
            ...
            % Track structure learned Beta distrib. parameters of ancestors on the LAST trial in a block
            if ib_c(ic) < 1 && it < nt
...
fprintf(' LSE = %d \n',objFn);
fprintf('Particle filtering duration: %.2f seconds \n',toc);
