...
cfg_tgt = struct;
cfg_tgt.mtgt = 5; % difference between the higher and 0
...
params(1) = cfg_mod.beta;
params(2) = cfg_mod.gamma;
params(3) = cfg_mod.kappa;
params(4) = cfg_mod.zeta;
...
%global datasource;
datasource  = 'sim';
fitcond     = {'rep'}; % Specify the experimental condition {'rep','alt','rnd','all'}
...
if strcmpi(fitcond,'all')
    fitcondinloop = {'rep','alt','rnd'};
else
    fitcondinloop = fitcond;
end
...
sm_choices  = nan(nb_c,nt,np,nc);   % Results from softmax decision of particles
lfn         = nan(nb_c,nt,nc);      % Likelihood function
...
abs = zeros(nb_c,nt,2,np,nc);   % Last-trial beta distribution parameters for the final particles
...
abs_anc = zeros(nb_c,nt,2,np,nc);
psl_anc = zeros(nb_c,nt,np,nc);
prl_anc = zeros(nb_c,nt,np,nc);
s_anc   = zeros(nb_c,nt,np,nc);
q_anc   = zeros(nb_c,nt,np,nc);
%
mt_protoanc = nan(nb_c,nt,2,np,nc); % sign of KF tracked means for beta param calculations
...
for ib = 1:nb
    ...
    if ~ismember(ctype,fitcondinloop)
    ...
    if ib_c(ic) < 3
        ...
        abs(ib_c(ic),:,2,:,ic)  = 1;     % Beta distrib. parameters = 1 (initial)
    ...
    % The beginning of the rest of the blocks 
        ...
        if ic == 3
            ...
            abs(ib_c(ic),:,2,:,ic)  = 1;     %%% this should not have a huge effect in the filtering
        else
            % Obtain switch/stay from mt_protoanc
            signbminus2 = sign(mt_protoanc(ib_c(ic)-1,end,1,:,ic)); % final tracked mean 2 blocks ago
            signbminus1 = sign(mt_protoanc(ib_c(ic)-1,end,2,:,ic)); % final tracked mean 1 block ago
            ...
            % Apply decay
            abs(ib_c(ic),1,1,:,ic) = abs_anc(ib_c(ic)-1,end,1,:,ic) + abs_a.*(1-gamma); % α(i) = α(i-1) + (1-γ)*a 
            abs(ib_c(ic),1,2,:,ic) = abs_anc(ib_c(ic)-1,end,2,:,ic) + abs_b.*(1-gamma); % β(i) = β(i-1) + (1-γ)*b
            ...
            for ip = 1:np
                ...
                if sign(mt_anc(ib_c(ic)-1,end,ip,ic)) == 1
                    psl(ib_c(ic),1,ip,ic) = abs(ib_c(ic),1,1,ip,ic)./(abs(ib_c(ic),1,1,ip,ic)+abs(ib_c(ic),1,2,ip,ic));
                else
                    psl(ib_c(ic),1,ip,ic) = 1-(abs(ib_c(ic),1,1,ip,ic)./(abs(ib_c(ic),1,1,ip,ic)+abs(ib_c(ic),1,2,ip,ic)));
                end
            end
        end
    
        % takes effect on ib_c = 3, making fn(3) = fn(2) %debug
        mt_protoanc(ib_c(ic),1,1,:,ic) = mt_protoanc(ib_c(ic)-1,end,2,:,ic);
...
%        disp(['mp: ' num2str(reshape(mp(ib_c(ic),it,:,ic),1,np))]); % debug
        ...
        if dv(ib_c(ic),it,1,ic) == shapeset(1)      % simulated response = correct shape
            
        elseif dv(ib_c(ic),it,1,ic) == shapeset(2)  % simulated response = incorrect shape
        ...
        for ip = 1:np
            sm_choices(ib_c(ic),it,ip,ic) = randsample([1 0], 1, true, [wdv(ib_c(ic),it,ip,ic) 1-wdv(ib_c(ic),it,ip,ic)]);
        end
        sum(sm_choices(ib_c(ic),it,:,ic))/np
        lfn(ib_c(ic),it,ic) = sum(sm_choices(ib_c(ic),it,:,ic))/np;
      ...
      % Filtering step
        ...
        if sum(rs_mult) < np
            ...
            if ~(ib_c(ic) == 1) && ~(it == 1)
                mt_protoanc(ib_c(ic),it,1,rs_range,ic) = mt_protoanc(ib_c(ic),it-1,1,impovs,ic);
                mt_protoanc(ib_c(ic),it,2,rs_range,ic) = mt_protoanc(ib_c(ic),it-1,2,impovs,ic);
            end
        ...
        for ia = 1:length(index_anc)
            ...
            if rs_mult(index_anc(ia)) == 0
                ...
                if it == 1
                    abs_anc(ib_c(ic),it,1,rs_range,ic) = abs(ib_c(ic),it,1,index_anc(ia),ic);
                    abs_anc(ib_c(ic),it,2,rs_range,ic) = abs(ib_c(ic),it,2,index_anc(ia),ic);
                else
                    abs_anc(ib_c(ic),it,1,rs_range,ic) = abs_anc(ib_c(ic),it-1,1,index_anc(ia),ic);
                    abs_anc(ib_c(ic),it,2,rs_range,ic) = abs_anc(ib_c(ic),it-1,2,index_anc(ia),ic);
                end

                % Propagate last KF tracking signs of the protoancestor
                if ~(ib_c(ic) == 1) && ~(it == 1)
                    mt_protoanc(ib_c(ic),it,1,rs_range,ic) = mt_protoanc(ib_c(ic),it-1,1,index_anc(ia),ic);
                    mt_protoanc(ib_c(ic),it,2,rs_range,ic) = mt_protoanc(ib_c(ic),it-1,2,index_anc(ia),ic);
                end
        ...
        if it == nt
            ...
            if ib_c(ic) == 1
                ...
                mt_protoanc(ib_c(ic),it,1,:,ic) = reshape(mt_anc(ib_c(ic),it,:,ic),size(mt_protoanc(ib_c(ic),it,1,:,ic)));
            % If the 2nd block instance of a given condition 
            elseif ib_c(ic) == 2
                % Initialize mt_protoanc(ib_c,2,it,:,ic)
                mt_protoanc(ib_c(ic),it,2,:,ic) = reshape(mt_anc(ib_c(ic),it,:,ic),size(mt_protoanc(ib_c(ic),it,2,:,ic)));
            else
                mt_protoanc(ib_c(ic),it,2,:,ic) = reshape(mt_anc(ib_c(ic),it,:,ic),size(mt_protoanc(ib_c(ic),it,2,:,ic)));
            end
        end
...
if strcmpi(fitcond{1},'all')
    objFn = -log(sum(sum(sum(exp(lfn(:,:,:))))));
else
    objFn = -log(sum(sum(exp(lfn(:,:,ic)))));
end
