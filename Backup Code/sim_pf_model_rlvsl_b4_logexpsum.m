...
%           : log(p(params|data))
%
% Version:  This version of sim_of_model_rlvsl considers the objective function as the LLR.
%           It does not sample the decision, but propagation and objective function 
%           calculations are based directly from the softmax probabilities. 
%           - 15 Jan 2020
...
for ib = 1:nb
    ...
    for it = 1:nt
        ...
        % Probability of action via softmax
        wdv(ib_c(ic),it,:,ic)	= 1./(1+exp(-beta*(mp(ib_c(ic),it,:,ic)-(1-mp(ib_c(ic),it,:,ic)))));
        ...
        if dv(ib_c(ic),it,1,ic) == shapeset(1)      % response = correct shape
            lls(ib_c(ic),it,ic)     = sum(log(wdv(ib_c(ic),it,:,ic)));
            wts(ib_c(ic),it,:,ic)   = wdv(ib_c(ic),it,:,ic)/sum(wdv(ib_c(ic),it,:,ic));
        elseif dv(ib_c(ic),it,1,ic) == shapeset(2)  % response = incorrect shape
            lls(ib_c(ic),it,ic)     = sum(log(1-wdv(ib_c(ic),it,:,ic)));
            wts(ib_c(ic),it,:,ic)   = (1-wdv(ib_c(ic),it,:,ic))/sum(1-wdv(ib_c(ic),it,:,ic));
        end
        % Debug code
        if ismember(1,isnan(lls(ib_c(ic),it,ic)))
            error('found NaN in log likelihood calculation');
            
        elseif ismember(lls(ib_c(ic),it,ic), [Inf -Inf])
            objFn = 1e8;
            badParams = true;
            fprintf('Beta: %.2f, Gamma: %.2f, Kappa: %.2f, Zeta: %.2f ',params(1),params(2),params(3),params(4));
        ...
        elseif ismember(1,isnan(wts(ib_c(ic),it,:,ic))) % this means that no particle had any probability of choosing the action
            objFn = 1e8;
            badParams = true;
            fprintf('Beta: %d, Gamma: %d, Kappa: %d, Zeta: %d ',params(1),params(2),params(3),params(4));
        ...
        for ia = 1:length(ind_anc)
            ...
            try 
                ...
                % Debug code
                dvweights = wdv(ib_c(ic),it,:,ic)
                weights =   wdv(ib_c(ic),it,:,ic)/sum(wdv(ib_c(ic),it,:,ic))
                weights2 =  (1-wdv(ib_c(ic),it,:,ic))/sum(1-wdv(ib_c(ic),it,:,ic));
                reshapewts = reshape(weights*np,1,np)
                reshapewts2 = reshape(weights2*np,1,np)
                dvval= dv(ib_c(ic),it,1,ic)
                shapes = shapeset
                rs_mult
...
elseif strcmpi(fitcond{1},'all')
    objFn = -sum(sum(sum(lls(:,:,:))));
else
    objFn = -sum(sum(lls(:,:,ic)));
end
