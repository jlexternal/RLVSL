...
try
    ...
    keyquit = KbName('X'); % JL: changed abort key since i don't have an ESC - debug
    keyresp = KbName({'Q','P'});
    ...
    for iblk = 1:nblk %block loop
        ...
        % import values from experimental block structure
        blck = [];
        ...
        if isprac(iblk) % practice blocks
            if iblk == 1
        ...
        for itrl = 1:ntrl
            ...
            if resp(itrl) ~= 1
                fb_val = fb_val*-1; % sign the original value (from old distrib) based on choice
            end
...
function draw_levers  % 1-left,up; 2-left,down; 3-right,up; 4-right,down;
    if lever_up == true
        ...
        Screen('DrawTexture',video.h,lever_tex(3),[],lever_rec(3,:),[],[],[],0);
    else
        if key == 1 % left chosen
            % draw lever left in down
            Screen('DrawTexture',video.h,lever_tex(2),[],lever_rec(2,:),[],[],[],0);
            ...
            Screen('DrawTexture',video.h,lever_tex(4),[],lever_rec(4,:),[],[],[],0);
        end
    end
end
