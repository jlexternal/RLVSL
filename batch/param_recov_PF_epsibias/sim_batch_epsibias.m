...
for ip = 1:numel(param_sets)
    ...
    % check for NaN in response matrix
    if any(reshape(isnan(rt),[1 numel(rt)]))
        error('NaN found in response matrix!');
    end
...
%% check for NaN in old dataset for quick batch distribution


load('data_sim_epsibias_03092020');
sim_struct = sim_data.sim_struct;
badsims = [];
badbatch = [];
nbatch = 48;
nperbatch = floor(numel(sim_struct)/nbatch);
for i = 1:numel(sim_struct)
    resp = sim_struct(i).resp;
    if any(reshape(isnan(resp),[1 numel(resp)]))
        badsims = cat(2,badsims,i);
        
       if ~ismember(floor(i/nperbatch)+1,badbatch)
           badbatch = cat(2,badbatch,floor(i/nperbatch)+1);
       end
