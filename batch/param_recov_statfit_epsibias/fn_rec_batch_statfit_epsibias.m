function fn_rec_batch_statfit_epsibias(ibatch)
...
% ***** REQUIRED: Manual user input for proper functionality ***** %
    ...
    %       working directory
    load('data_sim_statfit_epsibias_10092020.mat','sim_struct','param_sets'); % load sim data (sim_struct, param_set)
...
for ip = idx_batch(ibatch,1):idx_batch(ibatch,end)
    ...
    for isim = 1:sim_struct(ip).ns
        cfg = [];
        p_base = struct;
        for fn = fieldnames(sim_struct(ip).pb)'
            p_i = sim_struct(ip).pb.(fn{1});
            p_base.(fn{1}) = p_i(:,:,isim);
        end
        cfg.p_base = p_base;
        cfg.firstresp = sim_struct(ip).resp(:,1,isim);
        ...
        out_fit{ip,isim} = fit_stats_epsibias(cfg); % fit the model
    end
end

savename = ['out_rec_statfit_epsibias_' num2str(nbatch) '_' num2str(sprintf('%02d',ibatch))];
