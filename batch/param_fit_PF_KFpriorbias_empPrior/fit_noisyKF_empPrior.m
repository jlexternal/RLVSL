...
pnam{1} = 'kini';
pmin(1) = norminv(0.001,ep(1,1),ep(1,2));
pmax(1) = norminv(0.999,ep(1,1),ep(1,2));
pini(1) = normstat(ep(1,1),ep(1,2));
pplb(1) = norminv(0.1587,ep(1,1),ep(1,2));
ppub(1) = norminv(0.8413,ep(1,1),ep(1,2));
pfun{1} = @(x)normpdf(x,ep(1,1),ep(1,2));
% 2/ asymptotic Kalman gain
pnam{2} = 'kinf';
pmin(2) = norminv(0.001,ep(2,1),ep(2,2));
pmax(2) = norminv(0.999,ep(2,1),ep(2,2));
pini(2) = normstat(ep(2,1),ep(2,2));
pplb(2) = norminv(0.1587,ep(2,1),ep(2,2));
ppub(2) = norminv(0.8413,ep(2,1),ep(2,2));
pfun{2} = @(x)normpdf(x,ep(2,1),ep(2,2));
% 3/ learning noise
pnam{3} = 'zeta';
pmin(3) = norminv(0.001,ep(3,1),ep(3,2));
pmax(3) = norminv(0.999,ep(3,1),ep(3,2));
pini(3) = normstat(ep(3,1),ep(3,2));
pplb(3) = norminv(0.1587,ep(3,1),ep(3,2));
ppub(3) = norminv(0.8413,ep(3,1),ep(3,2));
pfun{3} = @(x)normpdf(x,ep(3,1),ep(3,2));
...
switch cscheme
    case 'qvs'
        ...
        ppub(4) = 0.5;
    case 'ths'
        pmin(4) = norminv(0.001,ep(4,1),ep(4,2));
        pmax(4) = norminv(0.999,ep(4,1),ep(4,2));
        pini(4) = normstat(ep(4,1),ep(4,2));
        pplb(4) = norminv(0.1587,ep(4,1),ep(4,2));
        ppub(4) = norminv(0.8413,ep(4,1),ep(4,2));
        pfun{4} = @(x)normpdf(x,ep(4,1),ep(4,2));
end
...
out.output   = output;
    ...
    function [pt,mt,vt] = getp(kini,kinf,zeta,theta,epsi)
        ...
        kini = 1/(1+exp(-kini));
        kinf = 1/(1+exp(-kinf));
        zeta = bounds(3,2)/(exp(-zeta)+1);
        theta = bounds(4,2)/(exp(-theta)+1);
